...
class segmeasure():
    def __init__(self, img, method, wdt, ld_model):
...
class PositionPub():  
    ...
    def publisher(self):
        ...
        pub_gazlink = rospy.Publisher('gazlink', LinkState, queue_size=10)
        ...
        # while time.time() < (snap+5):
            # print(time.time() - (snap+5))
        p = Pose()
        p.position.x = self.pippo
        p.position.y = self.topolino
        p.position.z = 0.05
        # Make sure the quaternion is valid and normalized
        p.orientation.x = 0.0
        p.orientation.y = 0.0
        p.orientation.z = 0.55
        p.orientation.w = 1.0
...
def main():
    ...
    segmentation = segmeasure(ch_img, meth, wdt, md)
    RosPub = PositionPub()

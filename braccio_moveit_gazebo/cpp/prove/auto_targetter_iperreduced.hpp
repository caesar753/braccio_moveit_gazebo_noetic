...
std::vector<double> A = {1.0, 1.0, 0.8};
Arm3Link InvKin(A);
...
//     }
    ...
    void  getDown_by_part(double x,double y){
        std::pair<double, double> result;
        std::vector<double> xy;
        double s, phi;
        ...
        std::cout << "s is " << s << "and phi is " << phi <<std::endl;
        std::cout << "s_vec[0] is " << s_vec[0] <<std::endl;
    }

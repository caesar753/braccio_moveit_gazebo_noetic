...
#include <cmath>
#include <lbfgs.h>
#include <NGC.h> // Include the NGC library
...
public:
    ...
    static lbfgsfloatval_t evaluate(
        ...
        // Calculate the gradient using the NGC library
        std::vector<double> gradient = arm->compute_gradient(q);

        // Copy the gradient values to the 'g' array
        for (int i = 0; i < n; i++) {
            g[i] = gradient[i];
        }
    ...
    std::vector<double> compute_gradient(const lbfgsfloatval_t *q) {
        NGC::AnalyticalGradientCalculator gradient_calculator;
        NGC::Tape tape;

        // Create a computation tape to record the operations
        NGC::VariableVector vars(q, q + 3); // Assuming you have 3 parameters
        tape.record(vars);

        // Define your objective function using NGC operations
        NGC::VariableScalar fx = distance_to_default(q);

        // Compute the gradient
        std::vector<double> gradient(3);
        for (int i = 0; i < 3; i++) {
            gradient[i] = tape.gradient(fx, vars[i]);
        }
    ...
    std::vector<double> optimizeIK(double x, double min_y, double max_y, double end_angle) {
        ...
        lbfgsfloatval_t fx;
        int ret = lbfgs(param.max_iterations, q_values, &fx, evaluate, this);
